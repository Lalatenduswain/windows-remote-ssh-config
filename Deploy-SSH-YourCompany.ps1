...
<#
.SYNOPSIS
    Deploy SSH setup to multiple company Windows PCs (Pre-configured)

.DESCRIPTION
    Runs Setup-SSH-YourCompany.ps1 on remote machines via PSRemoting or locally
...
.EXAMPLE
    .\Deploy-SSH-YourCompany.ps1 -ComputerNames @("PC-001","PC-002","PC-003")

.EXAMPLE
    .\Deploy-SSH-YourCompany.ps1 -ComputerNames @(".")  # Local only
#>
...
$SetupScriptPath = ".\Setup-SSH-YourCompany.ps1"  # Pre-configured setup script
...
if (!(Test-Path $SetupScriptPath)) {
    ...
    Write-Host "Make sure Setup-SSH-YourCompany.ps1 is in the same directory." -ForegroundColor Yellow
    exit 1
}
...
foreach ($Computer in $ComputerNames) {
    ...
    try {
        if ($Computer -eq "." -or $Computer -eq "localhost" -or $Computer -eq $env:COMPUTERNAME) {
            ...
            & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $SetupScriptPath -SSHPort $SSHPort
            ...
            Invoke-Command -Session $Session -ScriptBlock {
                ...
                & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $ScriptPath -SSHPort $Port
            } -ArgumentList $RemoteScriptPath, $SSHPort -ErrorAction Stop
