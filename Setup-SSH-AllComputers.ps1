...
<#
.SYNOPSIS
    Automates OpenSSH Server setup across company Windows PCs
...
.PARAMETER PublicKey
    The SSH public key to add (required). Format: ssh-rsa AAAA...
...
.EXAMPLE
    .\Setup-SSH-AllComputers.ps1 -PublicKey "ssh-rsa AAAA... admin@company"
...
try {
    if ($Username) {
        ...
        $User = New-Object System.Security.Principal.NTAccount($env:COMPUTERNAME, $Username)

        # Remove all permissions and reset
...
Write-Host "Test SSH with: ssh -i <private-key> <user>@$env:COMPUTERNAME -p $SSHPort"
Write-Host ""
